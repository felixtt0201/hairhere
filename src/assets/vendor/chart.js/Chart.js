...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(function () { try { return require('moment'); } catch (e) { } }())
    : typeof define === 'function' && define.amd ? define(['require'], (require) => factory(function () { try { return require('moment'); } catch (e) { } }()))
  ...
  const conversions = createCommonjsModule((module) => {
    /* MIT license */
 ...
 */
  var helpers = {
    ...
    _deprecated(scope, value, previous, current) {
      if (value !== undefined) {
 ...
 */
  ...
  const { valueOrDefault } = helpers_core;
 ...
 */
  const helpers_options = {
	 ...
	 * @since 2.7.0
	 */
    resolve(inputs, context, index, info) {
      let cacheable = true;
      let i; let ilen; let
        value;
 ...
 */
  ...
  const core_animations = {
    ...
    cancelAnimation(chart) {
      const index = helpers$1.findIndex(this.animations, (animation) => animation.chart === chart);
    ...
    advance() {
      const { animations } = this;
      let animation; let chart; let numSteps; let
        nextStep;
      let i = 0;
  ...
  const { resolve } = helpers$1.options;
    ...
    arrayEvents.forEach((key) => {
      ...
      Object.defineProperty(array, key, {
        ...
        value() {
          ...
          helpers$1.each(array._chartjs.listeners, (object) => {
 ...
 */
  ...
  helpers$1.extend(DatasetController.prototype, {
	 ...
	 * @private
	 */
    _resolveDatasetElementOptions(element, hover) {
      const me = this;
      const { chart } = me;
	 ...
	 * @private
	 */
    _resolveDataElementOptions(element, index) {
      const me = this;
    ...
    _setDatasetHoverStyle() {
      const element = this.getMeta().dataset;
      const prev = {};
      let i; let ilen; let key; let keys; let hoverOptions; let
        model;
  ...
  const { defaultColor } = core_defaults.global;
  ...
  const element_point = core_element.extend({
    ...
    draw(chartArea) {
      ...
      const { y } = vm;
      const globalDefaults = core_defaults.global;
      const { defaultColor } = globalDefaults; // eslint-disable-line no-shadow
 ...
 */
  ...
  const controller_bar = core_datasetController.extend({
	 ...
	 * @private
	 */
    _updateElementGeometry(rectangle, index, reset, options) {
      const me = this;
	 ...
	 * @private
	 */
    calculateBarValuePixels(datasetIndex, index, options) {
      const me = this;
      const { chart } = me;
      ...
      const { minBarLength } = options;
      const { stacked } = scale.options;
      const { stack } = me.getMeta();
      ...
      const ilen = metasets.length;
      let i; let imeta; let ivalue; let base; let head; let size; let
        stackLength;
	 ...
	 * @private
	 */
    calculateBarIndexPixels(datasetIndex, index, ruler, options) {
      const me = this;
  ...
  const controller_bubble = core_datasetController.extend({
    ...
    update(reset) {
      ...
      // Update Points
      helpers$1.each(points, (point, index) => {
	 ...
	 * @private
	 */
    _resolveDataElementOptions(point, index) {
      const me = this;
      const { chart } = me;
      const dataset = me.getDataset();
      const custom = point.custom || {};
  ...
  core_defaults._set('doughnut', {
    ...
    legendCallback(chart) {
      ...
      const { datasets } = data;
      const { labels } = data;
      let i; let ilen; let listItem; let
        listItemSpan;
  ...
  const controller_doughnut = core_datasetController.extend({
    ...
    update(reset) {
      ...
      let maxWidth; let maxHeight; let i; let
        ilen;
    ...
    calculateTotal() {
      ...
      helpers$1.each(meta.data, (element, index) => {
    ...
    getMaxBorderWidth(arcs) {
      ...
      let max = 0;
      const { chart } = me;
      let i; let ilen; let meta; let arc; let controller; let options; let borderWidth; let
        hoverWidth;
  ...
  const controller_line = core_datasetController.extend({
	 ...
	 * @private
	 */
    _resolveDatasetElementOptions(element) {
      const me = this;
      const config = me._config;
      const custom = element.custom || {};
      const { options } = me.chart;
    ...
    calculatePointY(value, index, datasetIndex) {
      ...
      let sumPos = 0;
      let sumNeg = 0;
      let i; let ds; let dsMeta; let stackedRightValue; let rightValue; let metasets; let
        ilen;
  ...
  const controller_polarArea = core_datasetController.extend({
    ...
    setHoverStyle(arc) {
      ...
      const options = arc._options;
      const { getHoverColor } = helpers$1;
      const { valueOrDefault } = helpers$1;
 ...
 */
  function parseVisibleItems(chart, handler) {
    ...
    let metadata; let i; let j; let ilen; let jlen; let
      element;
 ...
 */
  ...
  const { extend } = helpers$1;

  function filterByPosition(array, position) {
    return helpers$1.where(array, (v) => v.pos === position);
  }
 ...
 */
  ...
  const core_layouts = {
    ...
    update(chart, width, height) {
      ...
      helpers$1.each(boxes.chartArea, (layout) => {
        const { box } = layout;
 ...
 */
  ...
  function watchForRender(node, handler) {
    ...
    helpers$1.each(ANIMATION_START_EVENTS, (type) => {
 ...
 */
  ...
  const platform_dom$2 = {
    ...
    releaseContext(context) {
      ...
      helpers$1.each(initial.style || {}, (value, key) => {
 ...
 */
  ...
  const core_scaleService = {
    ...
    addScalesToLayout(chart) {
      ...
      helpers$1.each(chart.scales, (scale) => {
 ...
 */
  function createTooltipItem(element) {
    ...
    const { controller } = element._chart.getDatasetMeta(datasetIndex);
 ...
 */
  ...
  const exports$4 = core_element.extend({
    ...
    getBody(tooltipItems, data) {
      ...
      helpers$1.each(tooltipItems, (tooltipItem) => {
        const bodyItem = {
          before: [],
          lines: [],
          after: [],
    ...
    drawTitle(pt, vm, ctx) {
      const { title } = vm;
      const { length } = title;
      let titleFontSize; let titleSpacing; let
        i;
    ...
    drawBody(pt, vm, ctx) {
      ...
      let bodyItem; let textColor; let labelColors; let lines; let i; let j; let ilen; let
        jlen;
 ...
 */
  ...
  helpers$1.extend(Chart.prototype, /** @lends Chart */ {
    ...
    ensureScalesHaveIDs() {
      ...
      helpers$1.each(scalesOptions.xAxes, (xAxisOptions, index) => {
        if (!xAxisOptions.id) {
          xAxisOptions.id = nextAvailableScaleId(scalesOptions.xAxes, 'x-axis-', index);
        }
      });

      helpers$1.each(scalesOptions.yAxes, (yAxisOptions, index) => {
        if (!yAxisOptions.id) {
    ...
    buildOrUpdateScales() {
      ...
      helpers$1.each(updated, (hasUpdated, id) => {
        if (!hasUpdated) {
          delete scales[id];
        }
      });
    ...
    resetElements() {
      const me = this;
      helpers$1.each(me.data.datasets, (dataset, datasetIndex) => {
	...
	*/
    ...
    update(config) {
      ...
      if (me.options.animation && me.options.animation.duration) {
        helpers$1.each(newControllers, (controller) => {
          controller.reset();
        });
      }
    ...
    bindEvents() {
      ...
      helpers$1.each(me.options.events, (type) => {
    ...
    unbindEvents() {
      ...
      helpers$1.each(listeners, (listener, type) => {
 ...
 */
  ...
  const core_helpers = function () {
    ...
    helpers$1.where = function (collection, filterCallback) {
      ...
      helpers$1.each(collection, (item) => {
    ...
    helpers$1.numberOfLabelLines = function (arrayOfThings) {
      let numberOfLines = 1;
      helpers$1.each(arrayOfThings, (thing) => {
 ...
 */
  ...
  const { isArray } = helpers$1;
  const { isNullOrUndef } = helpers$1;
  const valueOrDefault$a = helpers$1.valueOrDefault;
  const { valueAtIndexOrDefault } = helpers$1;
 ...
 */
  function computeLabelSizes(ctx, tickFonts, ticks, caches) {
    ...
    let i; let j; let jlen; let label; let tickFont; let fontString; let cache; let lineHeight; let width; let height; let nestedLabel; let widest; let
      highest;
  ...
  function calculateSpacing(majorIndices, ticks, axisLength, ticksLimit) {
    ...
    let factors; let factor; let i; let
      ilen;
  ...
  const Scale = core_element.extend({
	...
	*/
    ...
    update(maxWidth, maxHeight, margins) {
      ...
      let i; let ilen; let labels; let ticks; let
        samplingEnabled;
    ...
    calculateTickRotation() {
      ...
      let labelSizes; let maxLabelWidth; let maxLabelHeight; let maxWidth; let tickWidth; let maxHeight; let
        maxLabelDiagonal;
    ...
    _convertTicksToLabels(ticks) {
      const me = this;
      let labels; let i; let
        ilen;

      me.ticks = ticks.map((tick) => tick.value);
	...
	*/
    ...
    _autoSkip(ticks) {
      ...
      let i; let ilen; let spacing; let
        avgMajorSpacing;
    ...
    _computeGridLineItems(chartArea) {
      ...
      const { offsetGridLines } = gridLines;
      const isHorizontal = me.isHorizontal();
      ...
      let borderValue; let i; let tick; let lineValue; let
        alignedLineValue;
      let tx1; let ty1; let tx2; let ty2; let x1; let y1; let x2; let y2; let lineWidth; let lineColor; let borderDash; let
        borderDashOffset;
    ...
    _drawGrid(chartArea) {
      ...
      let width; let color; let i; let ilen; let
        item;
    ...
    _drawLabels() {
      ...
      let i; let j; let ilen; let jlen; let item; let tickFont; let label; let
        y;
  ...
  const scale_category = core_scale.extend({
    ...
    getPixelForValue(value, index, datasetIndex) {
      const me = this;
      let valueCategory; let labels; let
        idx;
 ...
 */
  function generateTicks(generationOptions, dataRange) {
    ...
    const { stepSize } = generationOptions;
    const unit = stepSize || 1;
    const maxNumSpaces = generationOptions.maxTicks - 1;
    const { min } = generationOptions;
    const { max } = generationOptions;
    const { precision } = generationOptions;
  ...
  const scale_linearbase = core_scale.extend({
    ...
    getTickLimit() {
      const me = this;
      const tickOpts = me.options.ticks;
      const { stepSize } = tickOpts;
      let { maxTicksLimit } = tickOpts;
      let maxTicks;
  ...
  const scale_linear = scale_linearbase.extend({
    determineDataLimits() {
      ...
      helpers$1.each(stacks, (stackValues) => {
 ...
 */
  ...
  const scale_logarithmic = core_scale.extend({
    determineDataLimits() {
      ...
      let datasetIndex; let meta; let value; let data; let i; let
        ilen;
      ...
      if (opts.stacked || hasStacks) {
        ...
        helpers$1.each(valuesPerStack, (valuesForType) => {
 ...
 */
  ...
  const scale_radialLinear = scale_linearbase.extend({
    ...
    determineDataLimits() {
      ...
      helpers$1.each(chart.data.datasets, (dataset, datasetIndex) => {
        if (chart.isDatasetVisible(datasetIndex)) {
          const meta = chart.getDatasetMeta(datasetIndex);

          helpers$1.each(dataset.data, (rawValue, index) => {
    ...
    _drawGrid() {
      ...
      if (gridLineOpts.display) {
        helpers$1.each(me.ticks, (label, index) => {
          if (index !== 0) {
 ...
 */
  ...
  const scale_time = core_scale.extend({
    ...
    determineDataLimits() {
      ...
      const datasets = [];
      const labels = [];
      let i; let j; let ilen; let jlen; let data; let timestamp; let
        labelsAdded;
  ...
  function computeCircularBoundary(source) {
    const scale = source.el._scale;
    const { options } = scale;
    const { length } = scale.chart.data.labels;
    const { fill } = source;
    const target = [];
    let start; let end; let center; let i; let
      point;
  ...
  function doFill(ctx, points, mapper, view, color, loop) {
    ...
    let len0 = 0;
    let len1 = 0;
    let i; let ilen; let index; let p0; let p1; let d0; let d1; let
      loopOffset;
  ...
  const plugin_filler = {
    ...
    beforeDatasetsDraw(chart) {
      const metasets = chart._getSortedVisibleDatasetMetas();
      const { ctx } = chart;
      let meta; let i; let el; let view; let points; let mapper; let
        color;
  ...
  const Legend = core_element.extend({
    ...
    fit() {
      ...
      if (isHorizontal) {
        ...
        helpers$1.each(me.legendItems, (legendItem, i) => {
 ...
 */
  ...
  core_controller.helpers.each(scales, (scale, type) => {
