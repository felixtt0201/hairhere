  ...
  function resolveCustomIconVersion() {
    ...
    if (iconName && isPrivateUnicode(iconName)) {
      if (kitConfig && kitConfig.iconUploads) {
        const { iconUploads } = kitConfig;
        const descriptiveIconName = Object.keys(iconUploads).find((key) => iconUploads[key] && iconUploads[key].u && iconUploads[key].u === toHex(iconName));

        if (descriptiveIconName) {
          return iconUploads[descriptiveIconName].v;
        }
      }
    } else if (kitConfig && kitConfig.iconUploads && kitConfig.iconUploads[iconName] && kitConfig.iconUploads[iconName].v) {
